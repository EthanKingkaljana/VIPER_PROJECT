...
async def api_maskrcnn(req: SegReq):
    ...
    try:
        json_p, mask_p, overlay_p, detection_p = maskrcnn_run(
            img_path,
            RESULT_DIR,
        ...
        # 👇 IMPORTANT: return detection as vis_url
        return {
            "status": "success",
            "tool": "MaskRCNN",
            "json_url": static_url(json_p, OUT),
            "vis_url": static_url(detection_p, OUT),  # ✅ keep frontend happy
            "mask_url": static_url(mask_p, OUT),
            "json_data": load_output_json(json_p)
        }
