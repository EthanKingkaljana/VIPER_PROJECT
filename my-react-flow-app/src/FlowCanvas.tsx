...
import { saveWorkflowToDB } from './lib/api';
      ...
      const runAllNodes = async () => {
        try {
          ...
          const executionPriority: Record<string, number> = {
            ...
            'otsu': 30, 'snake': 30,
            'deep': 35, 'deeplab': 35, 'mask': 35, 'maskrcnn': 35,
    ...
    return (
      <div className="relative flex-1 h-full flex flex-col">
        <div className="absolute z-10 top-2 right-2 flex gap-2">
          <button onClick={async () => { try { const snapshot = { name: `${currentTabName || "Workflow"} - ${new Date().toLocaleString()}`, nodes, edges, viewport: getViewport() }; await saveWorkflowToDB(snapshot); saveWorkflow(); alert("Workflow version saved ✔"); } catch (e) { console.error(e); alert("Failed to save workflow"); } }} className="px-3 py-1 rounded bg-slate-800/80 hover:bg-slate-700 text-xs border border-slate-600 shadow-sm text-white">💾 SAVE</button>
