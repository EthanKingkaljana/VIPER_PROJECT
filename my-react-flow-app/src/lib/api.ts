...
export async function runSAM(image_path: string, params?: any, signal?: AbortSignal) {
  const resp = await fetch(`${API_BASE}/api/segmentation/sam`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ image_path, params }),
    signal,
...
export async function saveWorkflowToDB(payload: {
  name: string;
  nodes: any[];
  edges: any[];
  viewport: any;
